...
The mental model distinguishes three levels of template definition. The **primary template** provides the default implementation. **Explicit specializations** (also called full specializations) provide an implementation for a specific set of template arguments, matching exactly. **Partial specializations** provide an implementation for a family of arguments that satisfy a certain pattern, available only for class templates, variable templates, and alias templates—not for function templates. This asymmetry between class and function templates is one of the most important constraints to understand about the specialization system.
